...
impl DeviceCodeStore for SqliteDeviceCodeStore {
    ...
    async fn get_device_code(&self, device_code: &str) -> Result<Option<DeviceCodeEntry>> {
        ...
        if let Some(row) = row {
            Ok(Some(DeviceCodeEntry {
                ...
                scope: row.get("scope"),
                authorized_user: row.get("authorized_user"),
                expires_at: DateTime::from_timestamp(row.get::<i64, _>("expires_at"), 0)
    ...
    async fn consume_device_code(&self, device_code: &str) -> Result<Option<String>> {
        ...
        if let Some(row) = row {
            // Check if expired
            let expires_at = DateTime::from_timestamp(row.get::<i64, _>("expires_at"), 0)
                .unwrap_or_else(|| Utc::now());
